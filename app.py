...
with st.expander("📋 Show/Hide Validation Rules", expanded=False):
    ...
    <div class='rules-box'>
        ...
        * Normal shifts: 9 hours | Ramadan: 7 hours.<br>
...
if st.button("🚀 Run Swap Check", use_container_width=True):
    ...
    for emp_num, config in swap_config.items():
        ...
        # --- NEW: APPLY EXEMPTION RULES ---
        # Exemption 1: Off Saturday (Current Week)
        # Exemption 2: Off Sunday (Next Week - based on the swapped schedule)
        ...
        if not (is_off_sat or is_off_sun):
            # Only check 12h rule if neither exemption applies
            dt_end = get_dt(7, shift_ends[config['cur_id']]) # Saturday
            dt_start = get_dt(8, shift_starts[config['next_id']]) # Sunday
    ...
    if is_success:
        ...
        for res in validation_results:
            ...
            else: html += "<div class='reason-item' style='color: #a5d6a7;'>✅ Schedule is safe.</div>"
