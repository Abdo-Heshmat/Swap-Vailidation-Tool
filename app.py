...
st.markdown("""
    ...
    /* Neutral Start Box (Black/Dark Grey Background) */
    ...
    /* Neutral End Box (White/Light Grey Background with Black Text) */
    .neutral-end-box {
    ...
    .status-container { padding: 25px; border-radius: 15px; margin-top: 20px; border: 3px solid black; }
    .approved { background-color: #f0f0f0; color: #000; }
    .rejected { background-color: #333; color: #fff; }
    </style>
    ...
    with col:
        ...
        for week in ["Current Week", "Next Week"]:
            with st.container(border=True):
                ...
                with t_col1:
                    st.write("Start")
                    # Start box is now formatted with the dark theme
                ...
                with t_col3:
                    ...
                    # End box is now black text on white background
                    st.markdown(f"<div class='neutral-end-box'>{e_time}</div>", unsafe_allow_html=True)
...
if st.button("✅ Check the Validation", use_container_width=True):
    ...
    if rest_hours >= 21:
        ...
        st.markdown(f"<div class='status-container rejected'><h2 style='text-align: center;'>❌ Swap Rejected</h2><p style='text-align: center;'>Rest Period: {rest_hours:.1f} hours (Min: 21h)</p></div>", unsafe_allow_html=True)
