...
if st.session_state.theme == 'dark':
    ...
    # Pure White Formal Theme - This fixes the "Black Boxes" issue
...
st.markdown(f"""
    ...
    /* Strict override to fix "Black Boxes" in Light Mode */
    ...
    .results-card {{ padding: 25px; border-radius: 15px; margin-top: 25px; border: 2px solid {brd}; text-align: center; }}
    </style>
...
for i, col in enumerate([c1, c2], 1):
    with col:
        ...
        st.text_input(f"Name", key=f"un{i}", placeholder=f"Employee {i} Name", label_visibility="collapsed")
        ...
        for wk in ["Current", "Next"]:
            with st.container(border=True):
                ...
                # OT Row (Next to Shift)
                otc1, otc2 = st.columns(2)
                with otc1:
                ...
                # Work OT Toggles
                ot1 = st.toggle(f"Work OT on First Off", key=f"do_ot1_{i}_{wk}", disabled=st.session_state.get(f"do_ot2_{i}_{wk}", False))
                ot2 = st.toggle(f"Work OT on Second Off", key=f"do_ot2_{i}_{wk}", disabled=st.session_state.get(f"do_ot1_{i}_{wk}", False))
...
if st.button("🚀 RUN COMPLIANCE CHECK"):
    ...
    for c in checks:
        ...
        if is_exempt:
            msgs.append("✅ Rest Rule: Passed (Weekend Break)")
        else:
            ...
            if gap >= 12: msgs.append(f"✅ Rest Rule: Passed ({gap:.1f}h Rest)")
                ...
                msgs.append(f"❌ Rest Rule: REJECTED! ({gap:.1f}h Rest is < 12h)")
                fail = True
    ...
    for r in results:
        st.markdown(f"**Results for {r['name']}:**")
