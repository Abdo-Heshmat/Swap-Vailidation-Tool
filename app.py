...
def get_dt(day_idx, time_str, is_end_time=False, start_time_str=None):
    ...
    # Midnight Correction Rule: PM end -> AM start = NEXT DAY calculation
    if is_end_time and start_time_str:
        ...
        if e_hour < s_hour: # The shift crossed midnight into the next day
            target_date += timedelta(days=1)
            
    time_obj = datetime.strptime(time_str, "%I %p").time()
    return datetime.combine(target_date, time_obj)

# --- UI Styling (Keeping your exact shape and layout) ---
...
with st.expander("📋 View Validation Rules & Exemptions"):
    ...
    <div class='rules-box'>
        ...
        * Week starts on Sunday and ends on Saturday.<br>
        * <b>Important:</b> PM end → AM start = NEXT DAY calculation.<br>
        <div class='exemption-box'>
            <b>⚠️ Exemption Rules:</b><br>
            1. If employee is off on Saturday (Day 7) in current week, 12-hour rest rule is WAIVED.<br>
            2. If employee is off on Sunday (Day 8) in next week, 12-hour rest rule is WAIVED.
        </div>
    </div>
    """, unsafe_allow_html=True)
...
for i, col in enumerate([col1, col2], 1):
    with col:
        ...
        st.text_input("", key=f"user_name_{i}", label_visibility="collapsed", placeholder=f"Employee {i} Name")
...
if st.button("🚀 Validate Swap", use_container_width=True):
    ...
    for emp_num, data in config.items():
        errs = []
        name = data['name'] if data['name'] else f"Emp {emp_num}"
        ...
        if rest < 12 and not is_exempt:
            errs.append(f"❌ Insufficient rest: **{rest:.1f} hours** (Ends {dt_end.strftime('%a %I %p')}, Starts {dt_start.strftime('%a %I %p')})")
        elif is_exempt:
            errs.append(f"✅ Exempt from 12h rule (Off Saturday/Sunday coverage)")
